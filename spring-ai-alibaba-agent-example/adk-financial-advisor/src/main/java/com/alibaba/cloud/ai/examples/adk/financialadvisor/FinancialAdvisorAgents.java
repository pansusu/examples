...
@Configuration
public class FinancialAdvisorAgents {

     private static final String FINANCIAL_COORDINATOR_PROMPT = """
...
""";
     ...
     private static final String TRADING_ANALYST_PROMPT = """
...
""";
     ...
     private static final String EXECUTION_ANALYST_PROMPT = """
...
""";
     ...
     private static final String RISK_ANALYST_PROMPT = """
...
""";
    ...
    @Bean
	public ReactAgent financialCoordinator(ChatModel chatModel) {
        ToolCallback googleSearch = FunctionToolCallback.builder("google_search", (String input) -> {
            try {
                ...
                if (input != null && input.trim().startsWith("{")) {
                    req = mapper.readValue(input, new TypeReference<Map<String, Object>>(){});
                } else {
                    req = new HashMap<>();
                    req.put("query", input == null ? "" : input);
                }

                String query = String.valueOf(req.getOrDefault("query", ""));
                int maxDays = parseInt(req.get("max_data_age_days"), 7);
                int count = parseInt(req.get("target_results_count"), 5);
                if (count < 1) count = 5;
                ...
                for (int i = 1; i <= count; i++) {
                    int daysAgo = Math.min(i, Math.max(0, maxDays));
                    LocalDate pub = today.minusDays(daysAgo);
                    Map<String, Object> r = new HashMap<>();
                    r.put("title", String.format(Locale.ROOT, "%s — Update %d", query.isBlank() ? "Market" : query.toUpperCase(Locale.ROOT), i));
                    r.put("url", String.format(Locale.ROOT, "https://news.example.com/%s/%d", (query.isBlank()?"market":query.toLowerCase(Locale.ROOT).replaceAll("\\s+","-")), i));
                    r.put("source", i % 2 == 0 ? "Reuters" : "Bloomberg");
                    r.put("author", i % 2 == 0 ? "Staff Reporter" : "Analyst Desk");
                    r.put("date_published", fmt.format(pub));
                    r.put("snippet", String.format(Locale.ROOT, "Recent coverage on %s: item %d within last %d days.", query.isBlank()?"the market":query, i, maxDays));
                    results.add(r);
                }
                ...
                return "{\"query\":\"\",\"results\":[],\"error\":\"mock search error\"}";
            }
        })
        .description("Google Search tool (mock). Input JSON: { query: string, max_data_age_days?: int, target_results_count?: int }. Returns results: [{title,url,source,author,date_published,snippet}].")
        .build();

        ReactAgent dataAnalyst = ReactAgent.builder()
            .name("data_analyst_agent")
            .description("Data analyst: use Google Search to compile market analysis.")
            .model(chatModel)
        ...
        ReactAgent tradingAnalyst = ReactAgent.builder()
            .name("trading_analyst_agent")
            .description("Trading analyst: generate multiple trading strategy outlines.")
            .model(chatModel)
            .instruction(TRADING_ANALYST_PROMPT)
            .outputKey("proposed_trading_strategies_output")
            .build();

        ReactAgent executionAnalyst = ReactAgent.builder()
            .name("execution_analyst_agent")
            .description("Execution analyst: produce a detailed execution plan.")
            .model(chatModel)
            .instruction(EXECUTION_ANALYST_PROMPT)
            .outputKey("execution_plan_output")
            .build();

        ReactAgent riskAnalyst = ReactAgent.builder()
            .name("risk_analyst_agent")
            .description("Risk analyst: provide comprehensive risk evaluation and mitigations.")
            .model(chatModel)
        ...
        return ReactAgent.builder()
            ...
            .model(chatModel)
            .description("guide users through a structured process to receive financial advice by orchestrating a series of expert subagents. help them analyze a market ticker, develop trading strategies, define execution plans, and evaluate the overall risk.")
            .instruction(FINANCIAL_COORDINATOR_PROMPT)
            .outputKey("financial_coordinator_output")
            .tools(
                AgentTool.getFunctionToolCallback(dataAnalyst),
                AgentTool.getFunctionToolCallback(tradingAnalyst),
                AgentTool.getFunctionToolCallback(executionAnalyst),
                AgentTool.getFunctionToolCallback(riskAnalyst)
            )
            .build();
        }
